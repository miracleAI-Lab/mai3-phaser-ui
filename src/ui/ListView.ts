...
export class ListView extends Container {
    ...
    constructor(scene: BaseScene, config: ListViewConfig) {
        ...
        this.draw();
        this.setPosition(
            (scene.sys.game.config.width as number) / 2 - this.config.width / 2,
            (scene.sys.game.config.height as number) / 2 - this.config.height / 2
        );
    }

    draw() {
        const { width, height, padding = 0, background } = this.config;
    ...
    private renderVisibleItems() {
        ...
        const { itemHeight = 50, padding = 0 } = this.config;
    ...
    public onDrag(pointer: Phaser.Input.Pointer, dragX: number, dragY: number) {
        const contentHeight = this._allItems.length * (this.config.itemHeight! + this.config.padding!);
        const viewHeight = this.config.height - this.config.padding! * 2;

        if (contentHeight > viewHeight) {
            const newY = Phaser.Math.Clamp(this._content.y + (dragY - pointer.prevPosition.y), viewHeight - contentHeight, 0);
            this._content.setY(newY + this.config.padding!);
            
            // 计算新的起始索引
            const newStartIndex = Math.floor(-newY / (this.config.itemHeight! + this.config.padding!));
    ...
    private updateScrollBar() {
        const { width, height, padding = 0 } = this.config;
        const contentHeight = this._allItems.length * (this.config.itemHeight! + padding);
